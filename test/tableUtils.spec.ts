...
import { generateAsciiT		expect(result).toContain('║ Name │ Age ║');
		expect(result).toContain('║ John │  25 ║');
		expect(result).toContain('║ Jane │  30 ║');
		expect(result).toContain('╔══════╤═════╗');
		expect(result).toContain('╚══════╧═════╝'); parseCSV, autoGenerateColumns } from '../src/tableUtils';
...
Jane,30`;
			...
			console.log('Debug - parseCSV lines:', result);
			console.log('Debug - parseCSV lines count:', result.length);
